...
var EditProfileDialog = (props) => {
  ...
  >
    ...
    >{timeZones.map((option) => <Select.Option
...
var [_one_decimal, default_two_decimal] = decimalPlaces;
var DecimalPlaces = ({ label, onChange, value, errorMessage, action }) => {
  ...
  >{decimalPlaces.map((option) => <Select2.Option
    key={`${option.value}`}
    title={option.label}
    value={option.value}
...
var MeasurementTypes = ({ label, onChange, value, errorMessage, action }) => {
  ...
  >{measurementTypes.map((option) => <Select3.Option
    key={`${option.value}`}
...
// ../vlncc-frontend/src/components/SiteTree/Location/LocationFormDialog.tsx
import measurementTypes2 from "../../../data/measurementTypes.json";
import decimals from "../../../data/decimalPlaces.json";
import "../zone-and-location-dialog.scss";
var [temperature] = measurementTypes2;
...
var [__, { value: defaultDecimalPlaceValue }] = decimals;
...
import "react-phone-input-2/lib/style.css";
import timeZones2 from "../../data/timeZones.json";
...
var SetUpProfile = (props) => {
  ...
  return <Form12 mode="wide" htmlId="signup-setup-profile-form">
    ...
    >{timeZones2.map((option) => <Select5.Option
...
var SelectedLocationsList = (props) => {
  ...
  const processData = (data, prevData, id) => {
    ...
    if (data && data.items) {
      ...
      const decimals2 = selectedNode.decimal_places;
      selectedNode.average = data.average?.toFixed(decimals2) || null;
      selectedNode.minimum = data.min?.toFixed(decimals2) || null;
      selectedNode.maximum = data.max?.toFixed(decimals2) || null;
      selectedNode.most_recent = data.recent?.toFixed(decimals2) || null;
